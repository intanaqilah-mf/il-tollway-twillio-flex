/*
    ...
    path: /spanish-inqueue-callback
    private: CHECKED
    
    Function Methods (mode)
     - main             => main entry point for callback flow
     - mainProcess      => process main menu voice / DTMF selection
...
const { esSayOptions, speechTimeout, server_tz } = require(callbackConfigPath);
const spanishPromptsPath = Runtime.getFunctions().spanishCSRHadOffPrompts.path;
const { CSRHandOff1dot4a, CSRHandOff1dot4b, CSRHandOff1dot5, CSRHandOff1dot6, CSRHandOff1dot7a, CSRHandOff1dot7b, CSRHandOff1dot8, CSRHandOff1dot9 } = require(spanishPromptsPath);
...
exports.handler = function (context, event, callback) {
    ...
    //    END CUSTOMIZATIONS
    let command;
    let queryStr = '&language=' + language + '&isHoliday=' + isHoliday + '&isHolidayTomorrow=' + isHolidayTomorrow + '&endOfDayTime=' + endOfDayTime + '&startOfDayTime=' + startOfDayTime + '&queueStartTime=' + queueStartTime + '&workflowSid=' + workflowSid + '&ewt=' + ewt
    ...
    async function createCallback(phone, taskInfo) {
        ...
        //         ''
        //     ),
        //     taskAttribute_callbackOriginationCallSid: getOrigTaskData(
        //         taskInfo.originalTaskData,
        //         'taskQueueSid',
        //         ''
        //     ),
        ...
        case 'main':
            {
                ...
                let mainMenuOptions = {
                    ...
                    //hints: 'Yes, No',
                    speechTimeout: 'auto',
                    action: domain + '/spanish-inqueue-callback?mode=mainProcess&cbphone=' + explodePhone('format', phone) + queryStr + '&attempt=' + attempt,
                ...
                gather.say(esSayOptions, CSRHandOff1dot4a);
                gather.say(esSayOptions, explodePhone('explode', phone));
                gather.say(esSayOptions, CSRHandOff1dot4b);
                callback(null, twiml);
                break;
            }
        ...
        case 'mainProcess':
            {
                if (event.SpeechResult) { command = event.SpeechResult.toLowerCase(); }
                    ...
                    twiml.redirect({
                        method: 'POST'
                    }, 'https://' + context.DOMAIN_NAME + '/spanish-queue-menu?mode=mainProcess' + queryStr);
                    callback(null, twiml);
                }

                if (command) {
                    if (command.includes("sí") || (command.includes("Sí")) || command == 1) {
                        // redirect to submitCalBack
                        temp = event.cbphone;
                        twiml.redirect(
                            domain +
                            '/spanish-inqueue-callback?mode=submitCallback&callsid=' +
                            callSid +
                            '&cbphone=' +
                        ...
                        twiml.redirect(
                            domain +
                            '/spanish-inqueue-callback?mode=newNumber&callsid=' +
                        ...
                        twiml.redirect(
                            domain +
                            '/spanish-queue-menu?mode=mainProcess&callsid=' +
                            callSid +
                            '&cbphone=' +
        ...
        case 'newNumber':
            {
                let newNumberMenuOptions = {
                    ...
                    speechTimeout: 'auto',
                    action: domain + '/spanish-inqueue-callback?mode=newNumberProcess' + queryStr + '&attempt=' + attempt + '&noAttempt=' + noAttempt,
                    actionOnEmptyResult: true,
                    method: 'POST'
                };
                ...
                gather.say(esSayOptions, CSRHandOff1dot5);
                callback(null, twiml);
                break;
            }
        ...
        case 'newNumberProcess':
            {
                ...
                if (digits && digits.length === 10) {
                    twiml.redirect(
                        domain +
                        '/spanish-inqueue-callback?mode=newNumberConfirm&callsid=' +
                        callSid +
                        '&cbphone=' +
                    ...
                    twiml.say(esSayOptions, CSRHandOff1dot6);
                    twiml.redirect(
                        domain +
                        '/spanish-inqueue-callback?mode=newNumber&callsid=' +
                    ...
                    twiml.say(esSayOptions, CSRHandOff1dot9);
                    twiml.redirect({
                        method: 'POST'
                    }, 'https://' + context.DOMAIN_NAME + '/spanish-queue-menu?mode=mainProcess' + queryStr);
                }
        ...
        case 'newNumberConfirmProcess':
            {
                ...
                if (command) {
                    if (command.includes("sí") || (command.includes("Sí")) || command == 1) {
                        ...
                        twiml.redirect(
                            domain +
                            '/spanish-queue-menu?mode=mainProcess' + queryStr);
                        callback(null, twiml);
                    }
                }
                callback(null, twiml);
        ...
        case 'submitCallback':
            {
                ...
                async function main() {
                    ...
                    try {
                        ...
                        console.log("Callback Task : " + cbTask);
                        //  hangup the call
                        twiml.say(esSayOptions, CSRHandOff1dot8);
                        twiml.hangup();
                    } catch (ee) {
                        ...
                        twiml.redirect(
                            domain +
                            '/spanish-inqueue-callback?mode=main&callsid=' +
        ...
        case 'newNumberConfirm':
            {
                let newNumberConfimrMenuOptions = {
                    ...
                    hints: '$OPERAND',
                    speechTimeout: 'auto',
                    action: domain + '/spanish-inqueue-callback?mode=newNumberConfirmProcess&cbphone=' + explodePhone('format', phone) + queryStr + '&noAttempt=' + noAttempt,
                    actionOnEmptyResult: true,
                    method: 'POST'
                };
                ...
                gather.say(esSayOptions, CSRHandOff1dot7a);
                gather.say(esSayOptions, explodePhone('explode', phone));
                gather.say(esSayOptions, CSRHandOff1dot7b);
                callback(null, twiml);
                break;
            }
        default:
